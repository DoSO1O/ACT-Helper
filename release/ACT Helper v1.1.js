...
class ACTHelper {
	constructor (root) {
		...
		this.audioPlayer = [ new ACTHelper.BGMPlayer(), new ACTHelper.SEPlayer() ];
	}

	playBGM (url = "", volume = 1) {
		this.audioPlayer[0].src = url,
		this.audioPlayer[0].volume = volume;
	}
	...
	playSE (url = "", volume = 1, doesControlVolume = false) {
		this.audioPlayer[1].src = url,
		this.audioPlayer[1].volume = volume;

		if (doesControlVolume) {
			let control1 = () => { this.audioPlayer[0] }
		}
	}

	static get BGMPlayer () {
		return class BGMPlayer extends Audio {
			constructor (url = "", defaultVolume = 1) {
				super(url);

				this.loop = true;
				this.defaultVolume = defaultVolume;
				
				this.addEventListener("loadeddata", () => { this.play() });
				this.addEventListener("ended", () => { this.volume = defaultVolume });
			}
		}
	}

	static get SEPlayer () {
		return class SEPlayer extends Audio {
			constructor (url = "") {
				super(url);
